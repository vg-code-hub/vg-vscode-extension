/*
 * @Author: zdd
 * @Date: 2023-05-30 18:24:02
 * @LastEditors: zdd
 * @LastEditTime: 2023-05-31 14:59:54
 * @FilePath: /vg-vscode-extension/src/commands/welcome.ts
 * @Description: 
 */
 ...
 * @param {*} resp 
 */
function invokeCallback(panel: vscode.WebviewPanel, message: Message, resp: Response) {
...
const messageHandler = {
    getConfig(global: Global, message: Message) {
        const result = vscode.workspace.getConfiguration().get(message.key) as Response;
        invokeCallback(global.panel, message, result);
    },
    setConfig(global: Global, message: Message) {
...
export function showWelcome(context: vscode.ExtensionContext) {
    context.subscriptions.push(vscode.commands.registerCommand('extension.demo.showWelcome', function (uri) {
        const panel = vscode.window.createWebviewPanel(
            ...
            vscode.ViewColumn.One, // 显示在编辑器的哪个部位
            {
                ...
                localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, 'lib')), vscode.Uri.file(path.join(context.extensionPath, 'src', 'view'))],
                enableScripts: true, // 启用JS，默认禁用
            }
        );
        let global = { panel };
        panel.webview.html = getWebViewContent(context, panel, 'src/view/custom-welcome.html');


        panel.webview.onDidReceiveMessage(message => {
            if (messageHandler[message.cmd as keyof typeof messageHandler])
                messageHandler[message.cmd as keyof typeof messageHandler](global, message);
            else
    ...
    const key = 'vgVscodeExtension.showTip';
