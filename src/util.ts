...
const util = {
    ...
    getProjectPath(document: vscode.TextDocument | null) {
        ...
        const currentFile = ((document.uri ? document.uri : document) as any).fsPath;
    ...
    findStrInFolder: function (folderPath: string, str: string) {
     ...
     */
    findStrInFile: function (filePath: string, reg: RegExp | string) {
     ...
     */
    getStrRangeInFile: function (filePath: string, str: RegExp | string) {
        var pos = this.findStrInFile(filePath, str);
        return new vscode.Range(new vscode.Position(pos.row, pos.col), new vscode.Position(pos.row, pos.col + str.toString().length));
    },
    /**
     * 简单的检测版本大小
     */
    checkVersion: function (version1: string, version2: string) {
        const versionNum1 = parseInt(version1.replace(/\./g, ''));
        const versionNum2 = parseInt(version2.replace(/\./g, ''));
     ...
     * @param relativePath 扩展中某个文件相对于根目录的路径，如 images/test.jpg
     */
    getExtensionFileVscodeResource: function (context: vscode.ExtensionContext, relativePath: string) {
    ...
    openFileInFinder: function (filePath: string) {
        ...
        // 如果是目录，直接打开就好
        if (fs.statSync(filePath).isDirectory()) { exec(`open ${filePath}`); }
        else {
            // 如果是文件，要分开处理
     ...
     */
    openFileInVscode: function (path: string, text: RegExp | string) {
    ...
    openJarByJdGui: function (jarPath: string) {
        // 如何选中文件有待完善
        const jdGuiPath = vscode.workspace.getConfiguration().get('eggHelper.jdGuiPath') as string;
        if (!jdGuiPath) {
