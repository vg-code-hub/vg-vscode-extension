...
import { TS_TYPE, INDENT, SwaggerConfig, getDirPath, getTsType, getClassName, filterPathName, isStandardResponse, isPaginationResponse, getParamObj } from "../../utils";
...
// ${this.overwrite ? 'Do not edit, next time generation will overwrite it!' : 'Next time generation will not overwrite it, but instead generate a suffix of (.vg) file with the same name.'}`;
  }
...
export async function ${methodName}(${params}): Promise<${returnType}> {
  ${returnType !== 'void' ? 'const res = ' : ''}await http.${method.toLowerCase()}(${functionArgs});${this.getReturnContent(value.successResponse, getClassName(_name, false))}
}
`;
  }
  ...
  getReturnContent(responses: JSONSchema | undefined, resClassName: string) {
    ...
    if (returnType.startsWith('PageResp<')) {
      ...
      return `\n${INDENT}return { ...res.data, list: res.data.list ? ${TS_TYPE.includes(subType) ? 'res.data.list' : `(res.data.list as any[]).map<${subType}>((v: any) => ${subType}.fromJson(v))`} : [] };`;
    }
    if (returnType.endsWith('[]')) {
      var subType = returnType.substring(0, returnType.length - 2);
      return `\n${INDENT}return res.data ? ${TS_TYPE.includes(subType) ? 'res.data' : `(res.data as any[]).map<${subType}>((v: any) => ${subType}.fromJson(v))`} : [];`;
    }
    if (!TS_TYPE.includes(returnType))
